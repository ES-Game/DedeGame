...
class LikeCommentChapter : Action<LikeCommentChapter.RV, Int>() {
    ...
    override fun onExecute(rv: RV): Int {
        ...
        return try {
            dedeRepo.likeCommentChapter(rv.chapterId, rv.commentId)
    ...
    private fun handleTokenRefreshAndRetry(dedeRepo: IDedeGameRepo, rv: RV): Int {
        ...
        val onSuccess = {
            try {
                result = dedeRepo.likeCommentChapter(rv.chapterId, rv.commentId)
            } catch (e: APIException) {
                exception = e
            } finally {
                latch.countDown()
