...
class RatingStoryAction : Action<RatingStoryAction.RV, Rating>() {
    ...
    override fun onExecute(rv: RV): Rating {
        val dedeRepo: IDedeGameRepo = RepoFactory.getDedeGameRepo()
        return try {
            dedeRepo.ratingStory(rv.storyId, rv.rating)
    ...
    private fun handleTokenRefreshAndRetry(dedeRepo: IDedeGameRepo, rv: RV): Rating {
        ...
        val onSuccess = {
            try {
                result = dedeRepo.ratingStory(rv.storyId, rv.rating)
            } catch (e: APIException) {
                exception = e
            } finally {
                latch.countDown()
