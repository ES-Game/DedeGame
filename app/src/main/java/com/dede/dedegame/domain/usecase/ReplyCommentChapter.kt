...
class ReplyCommentChapter : Action<ReplyCommentChapter.RV, Comment>() {
    ...
    override fun onExecute(rv: RV): Comment {
        ...
        return try {
            dedeRepo.replyCommentChapter(rv.chapterId, rv.comment, rv.parentId)
    ...
    private fun handleTokenRefreshAndRetry(dedeRepo: IDedeGameRepo, rv: RV): Comment {
        ...
        val onSuccess = {
            try {
                result = dedeRepo.replyCommentChapter(rv.chapterId, rv.comment, rv.parentId)
            } catch (e: APIException) {
                exception = e
            } finally {
                latch.countDown()
