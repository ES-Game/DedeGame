...
class SendCommentToChapter : Action<SendCommentToChapter.RV, Comment>() {
    ...
    override fun onExecute(rv: RV): Comment {
        ...
        return try {
            dedeRepo.sendCommentToChapter(rv.chapterId, rv.comment)
    ...
    private fun handleTokenRefreshAndRetry(dedeRepo: IDedeGameRepo, rv: RV): Comment {
        ...
        val onSuccess = {
            try {
                result = dedeRepo.sendCommentToChapter(rv.chapterId, rv.comment)
            } catch (e: APIException) {
                exception = e
            } finally {
                latch.countDown()
