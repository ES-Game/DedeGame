...
class UnLikeCommentChapter : Action<UnLikeCommentChapter.RV, Int>() {
    ...
    override fun onExecute(rv: RV): Int {
        ...
        return try {
            dedeRepo.unlikeCommentChapter(rv.chapterId, rv.commentId)
    ...
    private fun handleTokenRefreshAndRetry(dedeRepo: IDedeGameRepo, rv: RV): Int {
        ...
        val onSuccess = {
            try {
                result = dedeRepo.unlikeCommentChapter(rv.chapterId, rv.commentId)
            } catch (e: APIException) {
                exception = e
            } finally {
                latch.countDown()
