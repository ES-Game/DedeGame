...
class Mapper<S : Any, D : Any>(private val srcClass: KClass<S>, private val destClass: KClass<D>) : Converter<S, D>, PropertyConverter<S, D> {

    companion object {
        fun <I : Any, O : Any> setMapper(mapper: Mapper<I, O>) = object : Converter<Set<I>, Set<O>> {
            override fun invoke(data: Set<I>): Set<O> = data.map(mapper).toSet()
        }

        fun <I : Any, O : Any> listMapper(mapper: Mapper<I, O>) = object : Converter<List<I>, List<O>> {
    ...
    override fun invoke(src: S): D {
        val destInstance = destClass.primaryConstructor!!.call()
        ...
        afterConvertCallback?.let {
            it.invoke(src, destInstance)
        }
        return destInstance
    }
    ...
    inline fun <reified I : Any?, reified O : Any?> register(destProperty: KProperty<*>, crossinline converter: Converter<I, O>) {
